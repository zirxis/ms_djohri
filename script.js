...
const searchData = [
    ...
    { 
        title: 'النصوص العلمية', 
        content: 'نصوص تحليل علمي مقالات بحوث', 
        url: '#resources',
        category: 'موارد'
    },
    { 
        title: 'اختبارات تجريبية', 
        content: 'امتحانات تقييم اختبار تجريبي', 
        url: '#resources',
        category: 'موارد'
    },
    { 
        title: 'عن الأستاذة', 
        content: 'جوهري ميس معلمة خبرة تدريس', 
        url: '#about',
        category: 'معلومات'
    }
];
...
function handleContactForm(e) {
    ...
    // Validate form data
    if (!data.name.trim()) {
        showNotification('الرجاء إدخال الاسم الكامل', 'warning');
        return;
    }
    ...
    if (!data.subject) {
        showNotification('الرجاء اختيار موضوع الرسالة', 'warning');
        return;
    }
    
    if (!data.message.trim()) {
        showNotification('الرجاء كتابة رسالتك', 'warning');
        return;
    }
...
function initializeScrollAnimations() {
    const animatedElements = document.querySelectorAll('.topic-card, .unit-card, .stat-card, .about-section, .contact-section');
...
function initialize() {
    try {
        ...
        setTimeout(() => {
            showNotification('مرحباً بك في موقع الأستاذة ميس للعلوم الطبيعية! 🌸', 'success');
        }, 1000);
